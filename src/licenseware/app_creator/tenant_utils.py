...
class TenantUtils:
    ...
    def get_processing_status(self, tenant_id):
        ...
        [self._close_timed_out(f) for f in self._get_timed_out_files(tenant_id)]
    ...
    def get_activated_tenants(self):
        tenants_list = m.fetch(match='tenant_id', collection=self.utilization_collection_name)
    ...
    def get_last_update_dates(self):
        ...
        last_update_dates = m.aggregate(pipeline, collection=self.data_collection_name)
